...
bool writeHomingOffsets(const uint8_t* ids, uint8_t id_count, const int32_t* offsets); //複数
...
bool readBulkPosVel(const uint8_t* ids, uint8_t id_count, int32_t* positions, int32_t* velocities);       //20250929
...
int32_t velocityDegPerSecToRaw(float vel_degpersec);
float rawToCurrentMilliAmp(int16_t curr_raw);
int16_t currentMilliAmpToRaw(float curr_mA);


//移動
void moveRawPosition(uint8_t id, int32_t rawPos);  //「度」まで回転
void moveRawAngVel(uint8_t id, int32_t rawAngVel); //
void moveRawCurrent(uint8_t id, int16_t rawCurrent); 
