...
//-----------------------一つのdynamixelを動かす関数
void moveRawPosition(uint8_t id, int32_t rawPos) {    //position mode
  uint8_t dxl_error = 0;

  packetHandler->write4ByteTxRx(portHandler, id, ADDR_GOAL_POSITION, rawPos, &dxl_error);
}

void moveRawAngVel(uint8_t id, int32_t rawAngVel) {  //velocity mode
  uint8_t dxl_error = 0;

  packetHandler->write4ByteTxRx(portHandler, id, ADDR_GOAL_VELOCITY, rawAngVel, &dxl_error);
}


void moveRawCurrent(uint8_t id, int16_t rawCurrent) {  //velocity mode
  uint8_t dxl_error = 0;

  packetHandler->write4ByteTxRx(portHandler, id, ADDR_GOAL_CURRENT, rawCurrent, &dxl_error);
}


bool readRawPosition(uint8_t id, int32_t* rawpos) {
  uint8_t dxl_error = 0;
  uint32_t value = 0;
  int result = packetHandler->read4ByteTxRx(portHandler, id, ADDR_PRESENT_POSITION, &value, &dxl_error);

  *rawpos = (int32_t)value;
  return true;
}


//---------複数のdynamixelを同時にreadWriteするSync&Buld Read Write--------------------------------------
...
bool readBulkPosVel(const uint8_t* ids, uint8_t id_count, int32_t* positions, int32_t* velocities) {
  ...
  for (uint8_t i = 0; i < id_count; i++) {
    uint8_t id = ids[i]; 
    // readするパラメータの登録。 groupBulkでは、同じidに対しては複数のaddPramが使えない。
    if (!groupBulkRead.addParam(id, ADDR_PRESENT_VELOCITY, 8)) return false;   //現在速度ADDR(4)+位置(4)=8
  }
  ...
  for (uint8_t i = 0; i < id_count; i++) {
    uint8_t id = ids[i];
    if (!groupBulkRead.isAvailable(id, ADDR_PRESENT_VELOCITY, 8)) return false;
    velocities[i]= groupBulkRead.getData(id, ADDR_PRESENT_VELOCITY, LEN_VELOCITY);
    positions[i] = groupBulkRead.getData(id, ADDR_PRESENT_POSITION, LEN_POSITION);
  }
...
//-------------変換 --------------------------------
float rawToDegree(int32_t pos_raw) { return (pos_raw * 360.0) / 4096.0; }
int32_t degreeToRaw(float degree) { return round(degree * 4096.0 / 360.0);}
float rawToVelocityDegPerSec(int32_t vel_raw) { return vel_raw * 0.229 * 6.0; }// 0.229 RPM → deg/s
int32_t velocityDegPerSecToRaw(float vel_degpersec) { return vel_degpersec/( 0.229 * 6.0);} // deg/s --> 0.229 RPM
float rawToCurrentMilliAmp(int16_t curr_raw) { return curr_raw * 2.69;}
int16_t currentMilliAmpToRaw(float curr_mA) {  return curr_mA/2.69;}    //mA --> count
